...
export function translatorError(key, language) {
  ...
  switch (language) {
    case "en":
      result = translationDictErrEN[key];
      break;
    case "it":
      result = translationDictErrIT[key];
      break;
    case "es":
      result = translationDictErrES[key];
      break;
    case "fr":
      result = translationDictErrFR[key];
      break;
    case "de":
      result = translationDictErrDE[key];
      break;
    default:
      result = key;
  }
...
export async function showError(key) {
  const language = await fetch("/user/language", {
    headers: {
      "Content-Type": "application/json",
    },
  });
  const message = translatorError(key, await language.text());
  Swal.fire({
    icon: "error",
    title: "Oops...",
    text: message,
